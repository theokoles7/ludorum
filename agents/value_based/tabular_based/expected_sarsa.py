...
class ExpectedSARSAAgent(TabularBasedAgent):
    ...
    def _update_(self,
        ...
        if done:
            ...
            for next_action in range(self._action_size_):
                if next_action == greedy_action:
                    # Greedy (highest Q-value): (1-ε) + ε/|A|
                    prob:   float = (
                                        (1 - self._exploration_rate_) + 
                                        (self._exploration_rate_ / self._action_size_)
                                    )
                else:
                    # If greedy action is not taken: ε/|A|
                    prob = self._exploration_rate_ / self._action_size_
        ...
        self._q_table_[state][action] +=    (
                                                ...
                                                (
                                                    ...
                                                    expected_q - self._q_table_[state][action]
                                                )
                                            )
